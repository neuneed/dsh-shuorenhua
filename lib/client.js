...
function ShuorenhuaModal({
  ...
  const readCachedResult = (0, import_react.useCallback)(async (text) => {
    try {
  ...
  (0, import_react.useEffect)(() => {
    ...
    setError(null);
    let cancelled = false;
    readCachedResult(originalText).then((cached) => {
      if (cancelled) return;
      if (cached) {
...
var pinnedOriginals = /* @__PURE__ */ new Map();
var PINNED_CAP = 200;
function pinOriginal(messageId, text) {
  ...
  if (pinnedOriginals.size > PINNED_CAP) {
    const oldestKey = pinnedOriginals.keys().next().value;
    if (oldestKey !== void 0) pinnedOriginals.delete(oldestKey);
  }
  return text;
}
function ShuorenhuaButton({
        ...
        for (const node of nodes) {
          ...
          if (node.kind === "turn-tail") {
            ...
            if (closing?.finalNode?.messageId === messageId) {
              const targetBlocks = Array.isArray(closing.finalNode.blocks) && closing.finalNode.blocks || Array.isArray(closing.steps) && closing.steps.map((s) => s?.data).find((d) => d?.finalNode?.messageId === messageId)?.blocks || Array.isArray(closing.message?.blocks) && closing.message.blocks;
              if (targetBlocks) {
                const text = targetBlocks.flatMap((b) => b.kind === "text" ? [b.text] : []).join("");
  ...
  const resolveTargetText = (0, import_react2.useCallback)(() => {
    ...
    if (buttonRef.current && typeof document !== "undefined") {
      try {
        let cursor = buttonRef.current.parentElement;
        for (let depth = 0; cursor && depth < 4; depth++) {
          const sibling = cursor.previousElementSibling;
...
var zh = {
  ...
  "modal.desc": "AI \u5B9E\u65F6\u91CD\u5199\u771F\u5B9E\u3001\u901A\u4FD7\u3001\u65E0\u5957\u8BDD\u7684\u81EA\u7136\u4EBA\u8BDD",
...
var en = {
  ...
  "modal.desc": "AI rewrites fluff into authentic, concise, and natural human speech in real time.",
